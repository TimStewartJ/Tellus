...
public class EarthCustomizeScreen extends EarthCustomizeScreenVersionCompat {
   ...
   public EarthGeneratorSettings applyPreviewAutoAdjust(TerrainPreview.PreviewInfo info) {
      ...
      int targetMinSurface = scaledSurfaceY(
         ...
         current.spawnLatitude(), current.automaticHeightScaling()
      );
   ...
   private static Registry<LevelStem> updateDatapackDimensions(
      ...
      for (Entry<ResourceKey<LevelStem>, LevelStem> entry : entries) {
         ...
         registerCopy(source, copy, key, updatedStem);
      }
   ...
   private static WorldDimensions updateDimensions(
      ...
      ChunkGenerator safeOverworldGenerator = Objects.requireNonNull(overworldGenerator, "overworldGenerator");
      RegistryLookup<DimensionType> dimensionTypesChecked = Objects.requireNonNull(dimensionTypes, "dimensionTypes");
      return updateDimensionsForVersion(
         dimensions,
         safeOverworldHolder,
         safeOverworldGenerator,
         dimensionTypesChecked,
         source -> updateDatapackDimensions(source, safeOverworldHolder, safeOverworldGenerator, dimensionTypesChecked)
      );
   }
   ...
   public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
      this.renderVersionBackground(graphics, mouseX, mouseY, delta);
   ...
   private List<EarthCustomizeScreen.CategoryDefinition> createCategories() {
      ...
      EarthCustomizeScreen.CategoryDefinition biomesCategory = new EarthCustomizeScreen.CategoryDefinition(
         "biomes", biomeSettings(RandomBiomeCatalog.legacyOverworldBiomeIds())
   ...
   private static boolean canFitPreviewAtWorldScale(EarthGeneratorSettings settings, TerrainPreview.PreviewInfo info, double worldScale) {
      int minBase = scaledSurfaceY(
         info.minElevationMeters(), worldScale, settings.terrestrialHeightScale(), settings.oceanicHeightScale(), 0,
         settings.spawnLatitude(), settings.automaticHeightScaling()
      );
      int maxBase = scaledSurfaceY(
         info.maxElevationMeters(), worldScale, settings.terrestrialHeightScale(), settings.oceanicHeightScale(), 0,
         settings.spawnLatitude(), settings.automaticHeightScaling()
      );
         ...
         if (!layerAccess.lookup(Registries.DIMENSION_TYPE).isEmpty()) {
            ...
            for (Entry<ResourceKey<DimensionType>, DimensionType> entry : entries) {
               ...
               if (!key.equals(nonNullTargetKey)) {
                  ...
                  registerCopy(source, copy, key, value);
               }
            }

            registerUpdatedDimensionType(source, copy, nonNullTargetKey, updatedType);
      ...
      if ("world".equals(category.getId())) {
         ...
         for (EarthCustomizeScreen.SettingDefinition setting : category.getSettings()) {
            if (setting instanceof EarthCustomizeScreen.SliderDefinition slider && isExperimentalAltitudeControl(slider.key)) {
               slider.forceDisabled(experimentalIncreaseHeight);
            }
         }
   ...
   private final class AutoAdjustDefinition implements EarthCustomizeScreen.SettingDefinition {
      ...
      public AbstractWidget createWidget(Runnable onChange) {
         boolean disabled = EarthCustomizeScreen.this.findToggleValue(
            "experimental_increase_height", EarthGeneratorSettings.DEFAULT.experimentalIncreaseHeight()
         );
   ...
   private static final class CacheActionWidget extends AbstractWidget {
      ...
      protected void renderWidget( GuiGraphics graphics, int mouseX, int mouseY, float delta) {
         ...
         ClientMinecraftCompat.setWidgetHeight(this.button, this.height);
   ...
   private static final class CacheProgressWidget extends AbstractWidget {
      ...
      protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
         ...
         if (snapshot.status() == TellusCacheManager.Status.READY) {
            Component label = Component.translatable("tellus.cache.ready");
            graphics.drawCenteredString(font, label, this.getX() + this.width / 2, this.getY() + (this.height - 9) / 2, READY_TEXT_COLOR);
            return;
         }
         if (snapshot.status() == TellusCacheManager.Status.FAILED) {
            Component label = Component.translatable("tellus.cache.failed");
            graphics.drawCenteredString(font, label, this.getX() + this.width / 2, this.getY() + (this.height - 9) / 2, FAILED_TEXT_COLOR);
            return;
         }
         ...
         if (deleting) {
            ...
            ClientMinecraftCompat.setWidgetHeight(this.cancelButton, this.height);
         ...
         graphics.drawString(font, fitCacheProgressText(font, label, labelWidth), contentLeft, this.getY() + 1, TEXT_COLOR, false);
         graphics.drawString(font, percent, percentX, this.getY() + 1, TEXT_COLOR, false);
         ...
         if (barWidth > 1) {
            ...
            graphics.renderOutline(barX, barY, barWidth, 4, BAR_BORDER);
   ...
   private static final class CacheEntryWidget extends AbstractWidget {
      ...
      protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
         ...
         if (this.allowDelete) {
            ...
            ClientMinecraftCompat.setWidgetHeight(this.deleteButton, this.height);
   ...
   private static final class DualButtonWidget extends AbstractWidget {
      ...
      protected void renderWidget( GuiGraphics graphics, int mouseX, int mouseY, float delta) {
         ...
         ClientMinecraftCompat.setWidgetHeight(this.leftButton, this.height);
         this.rightButton.setX(x + leftWidth + 4);
         this.rightButton.setY(y);
         this.rightButton.setWidth(rightWidth);
         ClientMinecraftCompat.setWidgetHeight(this.rightButton, this.height);
