...
/** Renders managed-terrain download progress in the HUD. */
public final class ManagedTerrainDownloadOverlay {
   ...
   public static void render(GuiGraphics graphics) {
      ...
      String stage = Component.translatable(
         "tellus.managed_terrain.overlay.stage." + status.stage().name().toLowerCase(Locale.ROOT)
      ).getString();
      String title = Component.translatable("tellus.managed_terrain.overlay.title").getString() + " · " + stage;
      if (status.totalCells() > 0) {
         title += " · " + Math.round(status.progress() * 100.0) + "%";
      }
      String detail = detail(status);
      ...
      graphics.drawString(font, fit(font, detail, width - 12), x + 7, y + 15, 0xFFD0D7DE, false);
      graphics.fill(x + 3, y + 25, x + width, y + 28, 0xFF26313A);
      int progressWidth = (int)Math.round((width - 3) * status.progress());
      if (progressWidth > 0) {
         graphics.fill(x + 3, y + 25, x + 3 + progressWidth, y + 28, accent);
      }
   }
   ...
   private static String detail(ManagedTerrainDownloadStatus status) {
      ...
      return Component.translatable(
         ...
         status.totalCells(),
         status.renderRadiusChunks(),
         status.safetyRingChunks()
      ).getString();
   }
