...
- `/tellus water nearest [x z] [radius]`: Operator diagnostics. Reports the nearest mapped watercourse centreline (kind, Overture id, nearest point, channel width, mapped-order heading and following vertices) within `radius` blocks (default 512) of the source or the given column. Source vertex order is not presented as a flow claim. Reads Overture tiles blocking; meant for debugging and automated playtests.
...
- All supported Minecraft and loader targets use the 10 m global canopy-top-height product by Lang et al. for procedural tree dimensions when Ecological Trees & Vegetation is enabled.
...
- Newly generated custom trees and bushes use vanilla leaf decay: leaves stay while supported by nearby logs, including branches, and decay after that support is removed. Foliage without a valid support path is omitted during generation.
- Existing chunks keep any persistent leaves generated by older builds. These are not automatically converted because they cannot be reliably distinguished from player-placed leaves; changing the Custom Trees setting does not repair existing foliage.
